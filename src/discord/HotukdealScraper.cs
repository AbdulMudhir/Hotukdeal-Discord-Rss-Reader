...
namespace Hotukdeal
{
    ...
    public class HotukdealScraper
    {
        ...
        {   // get the parent comment node then get the inner childs and retreive their value
            ...
            string commentNode;
            string forumNumber = Parser.forumNumber(url);

            try
            {
                commentNode = deal.SelectSingleNode($".//a[@class  = 'cept-comment-link btn space--h-3 btn--mode-boxSec cept-comment-link-id-{forumNumber} ']").ChildNodes[1].InnerText;
            }
            catch (NullReferenceException)
            {
                commentNode = deal.SelectSingleNode($".//span[@class='cept-comment-link btn space--h-3 btn--mode-boxSec cept-comment-link-id-{forumNumber}']").ChildNodes[1].InnerText;
            }                                                          
            return commentNode;
